...
class HuggingFaceProvider(BaseLLMProvider):
    ...
    def call_api_batch(self, queries: List[Query]) -> List[LLMResponse]:
        """
        Concurrent batch using LangChain's batch() method.
        """
        if not queries:
            return []
        ...
        raw_responses = self.llm.batch(prompts)
        ...
        for query, response in zip(queries, raw_responses):
            ...
            content = response if isinstance(response, str) else str(response)
            usage = {
                "input_tokens": len(query.prompt.split()),
                "output_tokens": len(content.split()),
                "total_tokens": len(query.prompt.split()) + len(content.split()),
            }
