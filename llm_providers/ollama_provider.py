...
class OllamaProvider(BaseLLMProvider):
    ...
    def call_api_batch(self, queries: List[Query]) -> List[LLMResponse]:
        """
        Concurrent batch using a ThreadPoolExecutor to fire multiple HTTP
        requests to the Ollama server in parallel, returning results in order.
        """
        if not queries:
            return []
        from concurrent.futures import ThreadPoolExecutor
        with ThreadPoolExecutor(max_workers=min(len(queries), 6)) as executor:
            return list(executor.map(self.call_api, queries))
